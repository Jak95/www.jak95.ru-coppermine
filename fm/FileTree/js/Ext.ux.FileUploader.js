...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	 ...
	 */
	,jsonErrorText: Res.get('JsonErrorText')
	 ...
	 */
	,unknownErrorText: Res.get('UnknownErrorText')
	...
	,fireFinishEvents:function(options) {
		...
		if(true !== this.eventsSuspended && 0 === this.upCount) {
			this.stopProgress();
			this.fireEvent('allfinished', this, this.areAllFilesSuccessfullyUploaded());
		}
  ...
  , areAllFilesSuccessfullyUploaded : function() {
    var records = this.store.queryBy(function(r){return 'done' !== r.get('state');});
