...
Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    */
    confirmText: Res.get('ConfirmText')
    ...
    */
	, deleteText: Res.get('DeleteTooltip')
    ...
    */
	, overwriteText: Res.get('OverwriteText')
    ...
    */
	, reallyWantText: Res.get('ReallyWantText')
    ...
    */
	, initComponent: function() {
      ...
      var beforeAjaxRequest = function(conn, options) {
        if (!options.jsonData)
          options.jsonData = options.params;
      };

      Ext.Ajax.on('beforerequest', beforeAjaxRequest, this);
	    ...
	    // create loader
	    if (!this.loader) {
	        this.loader = new Ext.tree.TreeLoaderExt({
	            url: this.url
				, baseParams: { cmd: 'get' }
				, listeners: {
	    ...
	    if (true === this.topMenu) {
	        ...
	        if (this.treeEditor) {
              ...
              this.treeEditor.triggerEdit = this.treeEditor.triggerEdit.createInterceptor(function(node){
                if (node.attributes.name != node.text)
    ...
    */
	, onRender: function() {
	    ...
	    if (true === this.loadMask) {
            ...
            if (node == this.root)
				      this.bwrap.mask.defer(20, this.bwrap, [this.loadingText + '...', 'x-mask-loading']);
				  }
				, load: this.bwrap.unmask.createDelegate(this.bwrap)
				, loadexception: this.bwrap.unmask.createDelegate(this.bwrap)
	        });
	    }
    ...
    */
	, cmdCallback: function(options, success, response) {
	    ...
	    if (true === success) {
	        ...
	        try {
              o = Ext.data.WCFGetObjectsFromResponse(response);
	        }
	        catch (ex) {
	        ...
	        // process command failure
	            ...
	            switch (options.params.cmd) {
	                ...
	                case 'rename':
                        ...
                        options.node.attributes.name = options.oldName;
                        this.updateNodeText(options.node);
	                    }
              ...
              Ic.Ext.ErrorHandler.showErrors(o.Errors);
    ...
    */
	, createNewDir: function(node) {
	    ...
	    appendNode.expand(false, false, function(n) {
            ...
            name: this.newdirText,
            text: this.newdirText,
            iconCls: 'folder',
            hasWriteFile: n.attributes.hasWriteFile,
            hasWriteFolder: n.attributes.hasWriteFolder,
            hasStatusFolder: n.attributes.hasStatusFolder }));
  ...
  , editProperties: function(node) {
    var win = new intercomp.FileCenter.EditPropertiesWindow({
      path: this.getPath(node)
    });
    win.on('save', function(folder) {
      this.onFolderPropertiesSaved(node, folder);
    }, this);
    win.show();
  }
  , onFolderPropertiesSaved: function(node, folder) {
    node.attributes.title = (folder.Title ? folder.Title.toLocalString() : null);
    node.attributes.qtip = (folder.Description ? folder.Description.toLocalString() : null);
    this.updateNodeText(node);
    this.refreshQtip(node);
  }
  , refreshQtip: function(node) {
    var ui = node.getUI();
    if(ui.textNode.setAttributeNS)
      ui.textNode.setAttributeNS("ext", "qtip", node.attributes.qtip);
    else
      ui.textNode.setAttribute("ext:qtip", node.attributes.qtip);
  }
    // }}}
    // {{{
    /**
    ...
    */
    ...
    // }}}
    // {{{
  , updateNodeText: function(node) {
    if (node.attributes.title && node.attributes.title != node.attributes.name)
      node.setText.defer(10, node, [String.format('{0} ({1})', node.attributes.name, node.attributes.title)]);
    else
      node.setText.defer(10, node, [node.attributes.name]);
  }
  , onCancelEdit: function(editor, value, startValue) {
    this.updateNodeText(editor.editNode);
  }
    /**
    ...
    */
	, showContextMenu: function(node) {
      ...
      menu.render();
	    ...
	    menu.setItemDisabled('delete', node === this.root || !node.parentNode.attributes.hasWriteFolder || node.attributes.isVirtual);
	    menu.setItemDisabled('rename', this.readOnly || node === this.root || !node.parentNode.attributes.hasWriteFolder || node.attributes.isVirtual);
	    menu.setItemDisabled('newdir', this.readOnly || !node.attributes.hasWriteFolder);
      menu.setItemDisabled('editproperties', this.readOnly || !node.parentNode.attributes.hasWriteFolder || node.attributes.isVirtual)
      ...
      var uploadPanelMenuEl = menu.getItemByCmd('upload-panel').getEl().parent();
      uploadPanelMenuEl.setVisibilityMode(Ext.Element.DISPLAY);
      uploadPanelMenuEl.setVisible(this.enableUpload);
